...
class trans_decoder_block(nn.Module):
    """使用转置卷积的解码器块，结构与decoder_block相似"""
    ...
    def __init__(self, in_c, out_c, scale=2):
        ...
        # 转置卷积替代上采样
        self.up = nn.ConvTranspose2d(in_c, in_c, kernel_size=scale, stride=scale, padding=0)
        self.c1 = CBR(in_c + out_c, out_c, kernel_size=1, padding=0)
        self.c2 = CBR(out_c, out_c, act=False)
        self.c3 = CBR(out_c, out_c, act=False)
        self.c4 = CBR(out_c, out_c, kernel_size=1, padding=0, act=False)
...
class output_block_trans(nn.Module):
    """使用转置卷积的输出块，功能与output_block相同"""

    def __init__(self, in_c, out_c=1):
        super().__init__()

        self.up_2x2 = nn.ConvTranspose2d(in_c, in_c, kernel_size=2, stride=2, padding=0)
        self.up_4x4 = nn.ConvTranspose2d(in_c, in_c, kernel_size=4, stride=4, padding=0)
        ...
        self.final_up = nn.ConvTranspose2d(in_c, in_c, kernel_size=2, stride=2, padding=0)
...
class FBGFFPreprocess(nn.Module):
...
class ICFSeg(nn.Module):
    ...
    """ Decouple Layer """
    self.decouple_layer = FBD(1024, 128)

        """ Adjust the shape of decouple output """
    self.preprocess_fg4 = FBGFFPreprocess(128, 128, 1)  # 1/16
    self.preprocess_bg4 = FBGFFPreprocess(128, 128, 1)  # 1/16

    self.preprocess_fg3 = FBGFFPreprocess(128, 128, 2)  # 1/8
    self.preprocess_bg3 = FBGFFPreprocess(128, 128, 2)  # 1/8

    self.preprocess_fg2 = FBGFFPreprocess(128, 128, 4)  # 1/4
    self.preprocess_bg2 = FBGFFPreprocess(128, 128, 4)  # 1/4

    self.preprocess_fg1 = FBGFFPreprocess(128, 128, 8)  # 1/2
    self.preprocess_bg1 = FBGFFPreprocess(128, 128, 8)  # 1/2
    ...
    self.fbgff4 = FBGFF(128, 128, 4)
    self.fbgff3 = FBGFF(128 + 128, 128, 4)
    self.fbgff2 = FBGFF(128 + 128, 128, 4)
    self.fbgff1 = FBGFF(128 + 128, 128, 4)

        """ Decoder - 原始解码器使用双线性插值上采样 """
        ...
        """ 新增解码器 - 使用转置卷积上采样 """
        self.decoder_small_trans = trans_decoder_block(128, 128, scale=2)
        self.decoder_middle_trans = trans_decoder_block(128, 128, scale=2)
        self.decoder_large_trans = trans_decoder_block(128, 128, scale=2)

        """ Output Block """
        self.output_block = output_block(128, 1)

        """ 新增Output Block - 使用转置卷积 """
        self.output_block_trans = output_block_trans(128, 1)
    ...
    def forward(self, image):
        ...
        f4 = self.fbgff4(d4, f_fg4, f_bg4)
        f4_up = self.up2X(f4)
        f_4_3 = torch.cat([d3, f4_up], dim=1)
        f3 = self.fbgff3(f_4_3, f_fg3, f_bg3)
        f3_up = self.up2X(f3)
        f_3_2 = torch.cat([d2, f3_up], dim=1)
        f2 = self.fbgff2(f_3_2, f_fg2, f_bg2)
        f2_up = self.up2X(f2)
        f_2_1 = torch.cat([d1, f2_up], dim=1)
        f1 = self.fbgff1(f_2_1, f_fg1, f_bg1)

        """ 原始Decoder - 使用双线性插值上采样 """
        f_small = self.decoder_small(f2, f1)
        f_middle = self.decoder_middle(f3, f2)
        f_large = self.decoder_large(f4, f3)

        """ 新增Decoder - 使用转置卷积上采样 """
        f_small_trans = self.decoder_small_trans(f2, f1)
        f_middle_trans = self.decoder_middle_trans(f3, f2)
        f_large_trans = self.decoder_large_trans(f4, f3)

        """ 原始Output Block """
        mask = self.output_block(f_small, f_middle, f_large)

        """ 新增Output Block - 使用转置卷积 """
        mask_trans = self.output_block_trans(f_small_trans, f_middle_trans, f_large_trans)

        return mask, mask_trans, mask_fg, mask_bg, mask_uc
...
class TeacherICFDSeg(nn.Module):
    def __init__(self, H=256, W=256):
        super().__init__()
    ...
    self.decouple_layer = FBD(1024, 128)
        ...
        """ Adjust the shape of decouple output """
    self.preprocess_fg4 = FBGFFPreprocess(128, 128, 1)
    self.preprocess_bg4 = FBGFFPreprocess(128, 128, 1)
    self.preprocess_fg3 = FBGFFPreprocess(128, 128, 2)
    self.preprocess_bg3 = FBGFFPreprocess(128, 128, 2)
    self.preprocess_fg2 = FBGFFPreprocess(128, 128, 4)
    self.preprocess_bg2 = FBGFFPreprocess(128, 128, 4)
    self.preprocess_fg1 = FBGFFPreprocess(128, 128, 8)
    self.preprocess_bg1 = FBGFFPreprocess(128, 128, 8)
    ...
    def forward(self, image):
        ...
        f_bg3 = self.preprocess_bg3(f_bg)
        f_fg2 = self.preprocess_fg2(f_fg)
        f_bg2 = self.preprocess_bg2(f_fg)
        f_fg1 = self.preprocess_fg1(f_fg)
        f_bg1 = self.preprocess_bg1(f_bg)
